...
class MotorMsg(enum.Enum):
    ...
    ReadParam = 17
    WriteParam = 18
    SaveConfig = 22  # <-- ADD THIS LINE (22 is 0x16 from the manual)
...
class Client:
    ...
    # This is the NEW, CORRECT version. Replace the old one with this.
    def save_configuration(self, motor_id: int, motor_model=1):
        """
        Sends the command to save the current parameters to the motor's
        non-volatile memory (EEPROM), based on Communication Type 22 (0x16)
        from the RS03-EN manual.
        """
        msg_type_to_send = MotorMsg.SaveConfig
        data_to_send = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]
        self.bus.send(self._rs_msg(msg_type_to_send, self.host_can_id, motor_id, data_to_send))

        # Wait for any response from the motor. _recv() will timeout if none arrives.
        resp = self._recv()

        # --- Manual, more robust validation ---
        # Instead of parsing the whole message, we only check that the response
        # came from the motor we were talking to. We don't care what message type it is.
        _, resp_motor_id, _ = self._parse_resp_abitration_id(resp.arbitration_id)

        if resp_motor_id != motor_id:
            raise Exception(f"Sent Save command to motor {motor_id}, but received unexpected response from motor {resp_motor_id}")

        # If we get here, it means the correct motor acknowledged the command. We are done.
        print(f"  - Received confirmation from motor {motor_id}")
    ...
    def zero_pos(self, motor_id: int, motor_model=1) -> FeedbackResp:
        ...
        self.bus.send(self._rs_msg(MotorMsg.ZeroPos, self.host_can_id, motor_id, [0x01, 0, 0, 0, 0, 0, 0, 0]))
        resp = self._recv()
        return self._parse_feedback_resp(resp, motor_id, motor_model)